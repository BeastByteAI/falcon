...
class MultiModalEncoder(ScalerAndEncoder):
    ...
    def _get_date_tokenizer(self) -> BaseEstimator: 
        return SKLPipeline(steps=[('date_tokenizer', DateTimeTokenizer(format=r"%Y-%m-%d")), ('cast32', CastTransformer()), ('sc', MaxAbsScaler())])
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None: 
        ...
        for i, v in enumerate(self.mask):
            ...
            elif v == ColumnTypes.DATE_YMD_ISO8601:
                method = self._get_date_tokenizer()
            else:
