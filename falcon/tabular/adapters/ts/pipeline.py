...
class TSAdapterPipeline(Pipeline):
    ...
    def __init__(self, task: str, dataset_size: Tuple[int], mask: List[Any], wrapped_pipeline: Type[Pipeline], wrapped_pipeline_options: Dict, **kwargs: Any) -> None:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, *args: Any, **kwargs: Any) -> None:
        self._pipeline = []
        learner = TSAdapterLearner(self.task, self.dataset_size, self.mask, self.wrapped_pipeline, self.wrapped_pipeline_options)
