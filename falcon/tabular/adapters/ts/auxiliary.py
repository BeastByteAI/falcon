...
def _split_fn(X: np.ndarray, y:np.ndarray, eval_size: float = 0.2) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def _wrap_onnx(m: ModelProto) -> None:
    ...
    inputs = [h.make_tensor_value_info(new_names[i], TensorProto.FLOAT, [None, 1]) for i in range(len(input_names_inorder))]


    sample_mean = h.make_node('Mean', inputs = new_names, outputs = ['sample_mean'],  domain = '', name = 'calculate_sample_mean')
    additional_nodes.insert(0, sample_mean)
    reshape_shape  = h.make_tensor(name='shape_', data_type=TensorProto.INT64, dims=[2], vals=np.asarray([-1, 1]).astype(np.int64))
    m.graph.initializer.append(reshape_shape)
    reshape_mean = h.make_node('Reshape', inputs = ['sample_mean', 'shape_'], outputs = ['sample_mean_reshaped'],  domain = '', name = 'reshape_mean')
    additional_nodes.insert(0, reshape_mean)
    array_feature_extract_out_names = []
    for i in range(len(new_names)):
        node_out_name = f'sample_normalized_{i}'
        sub = h.make_node('Sub', inputs = [new_names[i], 'sample_mean_reshaped'], outputs = [node_out_name],  domain = '', name = f'normalize_sample_{i}')
        additional_nodes.insert(0, sub)
        array_feature_extract_out_names.append(node_out_name)

    reshape_output = h.make_node('Reshape', inputs = ['model_result', 'shape_'], outputs = ['model_result_reshaped'],  domain = '', name = 'reshape_model_result')
    additional_nodes.insert(0, reshape_output)
    add_ = h.make_node('Add', inputs = ['model_result_reshaped', 'sample_mean_reshaped'], outputs = ['result_denormalized'],  domain = '', name = 'add_mean')
    additional_nodes.insert(0, add_)
    squeeze = h.make_node('Squeeze', inputs = ['result_denormalized'], outputs = ['ts_output'],  domain = '', name = 'squeeze_result')
